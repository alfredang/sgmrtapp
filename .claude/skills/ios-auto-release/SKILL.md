...
## This project (SGMRT) — implemented config

Live pipeline: [.github/workflows/ios-release.yml](../../../.github/workflows/ios-release.yml).
Helper: [scripts/ci_submit.py](../../../scripts/ci_submit.py). CI screenshots (used only when a
new version is created): `ci/screenshots/APP_IPHONE_67/`. Release notes source: `CHANGELOG.md`.

| Setting | Value |
|---|---|
| `ASC_BUNDLE_ID` | `com.alfredang.sgmrt` |
| `ASC_APP_ID` | `6781980848` |
| `TEAM_ID` | `GU9WTSTX9M` |
| `SCHEME` / `PROJECT` | `SGMRT` / `SGMRT.xcodeproj` |
| `PROFILE_NAME` | `SGMRT App Store API` (App Store; embeds *iPhone Distribution: Alfred Ang*) |
| `SIGN_IDENTITY` | `iPhone Distribution` (matches the profile + `ExportOptions.plist`) |
| Trigger | push to `main` (ignores `**.md`, `.github/**`, `scripts/**`, `ci/screenshots/**`) + manual `workflow_dispatch` |

Repo secrets are set (`gh secret list`): `ASC_KEY_ID`, `ASC_ISSUER_ID`, `ASC_API_PRIVATE_KEY`,
`DIST_CERT_P12_BASE64`, `DIST_CERT_PASSWORD`, `PROVISIONING_PROFILE_BASE64`.

Notes specific to SGMRT:
- The `.xcodeproj` is generated by XcodeGen from `project.yml` but **committed**, so CI archives it
  directly (no `xcodegen` on the runner). If you edit `project.yml`, run `xcodegen generate` and
  commit the result before pushing.
- No CloudKit, no login → no account-deletion flow and no CloudKit schema deploy needed.
- App Privacy + age rating are set once in the ASC UI and carry across versions; CI never touches them.

Watch a run: `gh run watch --repo alfredang/sgmrtapp` or `gh run list --workflow=ios-release.yml`.
